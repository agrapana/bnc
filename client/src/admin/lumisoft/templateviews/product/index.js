...
const ProductScreen = (props) => {
    ...
    const [pagename] = useState('Product List');
    ...
    const [tablename] = useState('Product');
    ...
    useEffect(() => {
        if (getalldata && getalldata.getProduct && getalldata.getProduct.success) {
            let totaldata = getalldata.getProduct.product.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    ...
    useEffect(() => {
        if (alldata && alldata.length > 0) {
            ...
            resultsdata = alldata.map((item, index) => <ProductTable key={index} item={item} index={index} />)
        ...
        if (alldata) {
            results = alldata.filter(data =>
                (data.name.toLowerCase().indexOf(filterText.toLowerCase()) !== -1
                    || data.brand.name.toLowerCase().indexOf(filterText.toLowerCase()) !== -1))
            resultsdata = results.map((item, index) => <ProductTable item={item} index={index} key={index} editData={editData} />)
    ...
    const onRefreshHandler = async () => {
        try {
            loadingtableHandler(true);
            let alldatas = await dispatch(getProduct());
            let totaldata = await alldatas.payload.products.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
            var mydata = [];
            var resultsdata = [];
            var offset = (currentPage - 1) * pageSize;
            alldataHandler(totaldata);
            resultsdata = totaldata.map((item, index) => <ProductTable key={index} item={item} index={index} />)
            mydata = resultsdata.slice(offset, offset + pageSize);
            mydataHandler(mydata);
            dataCountHandler(resultsdata.length);
            loadingtableHandler(false);
    ...
    const editDataHandler = () => {
        props.history.push({
            pathname: '/admin/product/editdata',
            state: {
