...
const PortfolioScreen = (props) => {
    ...
    const [pagename] = useState('Portfolio List');
    ...
    const [tablename] = useState('Portfolio');
    ...
    useEffect(() => {
        if (getalldata && getalldata.getPortfolio && getalldata.getPortfolio.success) {
            let totaldata = getalldata.getPortfolio.portfolios.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    ...
    useEffect(() => {
        if (alldata && alldata.length > 0) {
            ...
            resultsdata = alldata.map((item, index) => <PortfolioTable key={index} item={item} index={index} />)
    ...
    useEffect(() => {
        ...
        if (alldata) {
            ...
            resultsdata = results.map((item, index) => <PortfolioTable item={item} index={index} key={index} editData={editData} />)
    ...
    const onRefreshHandler = async () => {
        try {
            loadingtableHandler(true);
            let alldatas = await dispatch(getPortfolio());
            let totaldata = await alldatas.payload.portfolios.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
            var mydata = [];
            var resultsdata = [];
            var offset = (currentPage - 1) * pageSize;
            alldataHandler(totaldata);
            resultsdata = totaldata.map((item, index) => <PortfolioTable key={index} item={item} index={index} />)
            mydata = resultsdata.slice(offset, offset + pageSize);
            mydataHandler(mydata);
            dataCountHandler(resultsdata.length);
            loadingtableHandler(false);
    ...
    const editDataHandler = () => {
        props.history.push({
            pathname: '/admin/portfolio/editdata',
            state: {
