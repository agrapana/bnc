...
const ServerScreen = (props) => {
    ...
    const [tablename] = useState('Server');
    ...
    useEffect(() => {
        if (getalldata && getalldata.getServers && getalldata.getServers.success) {
            let totaldata = getalldata.getServers.servers.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    ...
    useEffect(() => {
        if (alldata && alldata.length > 0) {
            ...
            resultsdata = alldata.map((item, index) => <ServerTable key={index} item={item} index={index} />)
        ...
        if (alldata) {
            results = alldata.filter(data =>
                (data.name.toLowerCase().indexOf(filterText.toLowerCase()) !== -1
                    ...
                    || data.ipaddress.toLowerCase().indexOf(filterText.toLowerCase()) !== -1))
            resultsdata = results.map((item, index) => <ServerTable item={item} index={index} key={index} editData={editData} />)
    ...
    const addNewHandler = () => {
        ...
        props.history.push('/admin/servers/addnew');
    }

    const onRefreshHandler = async () => {
        try {
            loadingtableHandler(true);
            let alldatas = await dispatch(getServers());
            let totaldata = await alldatas.payload.servers.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
            var mydata = [];
            var resultsdata = [];
            var offset = (currentPage - 1) * pageSize;
            alldataHandler(totaldata);
            resultsdata = totaldata.map((item, index) => <ServerTable key={index} item={item} index={index} />)
            mydata = resultsdata.slice(offset, offset + pageSize);
            mydataHandler(mydata);
            dataCountHandler(resultsdata.length);
            loadingtableHandler(false);
    ...
    const editDataHandler = () => {
        props.history.push({
            pathname: '/admin/servers/editdata',
            state: {
