...
const MasterApplicationScreen = (props) => {
    const { applicationprops } = useSelector(state => ({
        applicationprops: state.application
    }));
    const size = useWindowSize();
    const isMobile = size.width <= 767.98;
    const dispatch = useDispatch();
    const [loadingtable, loadingtableHandler] = useState(false);
    const [pagename] = useState('Application Lists');
    ...
    const [tablename] = useState('LumisoftApplication');
    ...
    useEffect(() => {
        if (applicationprops && applicationprops.getApplication && applicationprops.getApplication.success) {
            let totaldata = applicationprops.getApplication.applications.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        ...
        if (alldata) {
            results = alldata.filter(data =>
                (data.name.toLowerCase().indexOf(filterText.toLowerCase()) !== -1
                    || data.description.toLowerCase().indexOf(filterText.toLowerCase()) !== -1))
    ...
    const addNewHandler = () => {
        ...
        props.history.push('/admin/master/application/addnew');
    }
    const onRefreshHandler = async () => {
        try {
            loadingtableHandler(true);
            let alldatas = await dispatch(getApplication());
            let totaldata = await alldatas.payload.applications.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
            var mydata = [];
            var resultsdata = [];
            var offset = (currentPage - 1) * pageSize;
            alldataHandler(totaldata);
    ...
    const editDataHandler = () => {
        props.history.push({
            pathname: '/admin/master/application/editdata',
            state: {
...
export default MasterApplicationScreen;