...
import {
    ...
    updateSlider,
    clearUpdateSlider
} from '../../../store/actions/slider_action';
...
const SliderTableScreen = (props) => {
    ...
    const [formdata, formdataHandler] = useState({
        ...
        linktitle: {
            element: 'input',
            title: 'Button Title',
            value: editformdatastatus ? dataselected.linktitle : '',
            config: {
                name: 'nameInput',
                type: 'text',
        ...
        linkto: {
            ...
            value: editformdatastatus ? dataselected.linkto : '',
            config: {
                name: 'nameInput',
                type: 'text',
                placeholder: 'Enter button destination'
            },
            validation: {
                required: false
            },
    ...
    const updateForm = (element) => {
        const newFormdata = update(element, formdata, 'slider');
    ...
    const goBackToTable = () => {
        props.history.push('/admin/slider');
    }
    ...
    const submitData = (event) => {
        ...
        let dataToSubmit = generateData(formdata, 'slider');
        let formIsValid = isFormValid(formdata, 'slider');

        if (formIsValid) {
            dispatch(addSlider(dataToSubmit)).then(response => {
    ...
    const submitEditData = (event) => {
        ...
        let dataToSubmit2 = generateData(formdata, 'slideredit');
        let formIsValid2 = isFormValid(formdata, 'slideredit');
        let datachange = 0;
        ...
        if (datachange === 3) {
            ...
            if (formIsValid2) {
                ...
                dispatch(updateSlider(totaldataToSubmit)).then(response => {
                    if (response.payload.success) {
                        dispatch(clearUpdateSlider());
                        formSuccessHandler(true);
                        props.history.push('/admin/slider');
                    } else {
                        formErrorHandler(true);
    ...
    return (
        <div className="cardBody formstyle">
            ...
            }
            {
                props.buttonlink ?
                    <div className="row pb25">
                        <label className="col-md-2 col-xs-12 colFormLabel">{formdata.linktitle.title}</label>
                        <div className="col-md-10 col-xs-12">
                            <div className="iconPosition">
                                <FontAwesomeIcon
                                    icon={faAlignLeft}
            ...
            }
            {
                props.buttondestination ?
                    <div className="row pb25">
                        <label className="col-md-2 col-xs-12 colFormLabel">{formdata.linkto.title}</label>
                        <div className="col-md-10 col-xs-12">
                            <div className="iconPosition">
                                <FontAwesomeIcon
                                    icon={faAlignLeft}
...
export default SliderTableScreen;