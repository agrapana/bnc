...
const GalleryScreen = (props) => {
    ...
    const [tablename] = useState('Gallery');
    ...
    useEffect(() => {
        if (getalldata && getalldata.getGallery && getalldata.getGallery.success) {
            let totaldata = getalldata.getGallery.galleries.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    ...
    useEffect(() => {
        if (alldata && alldata.length > 0) {
            ...
            resultsdata = alldata.map((item, index) => <GalleryTable key={index} item={item} index={index} />)
        ...
        if (alldata) {
            results = alldata.filter(data =>
                (data.name.toLowerCase().indexOf(filterText.toLowerCase()) !== -1
                    ...
                    || data.category.name.toLowerCase().indexOf(filterText.toLowerCase()) !== -1))
            resultsdata = results.map((item, index) => <GalleryTable item={item} index={index} key={index} editData={editData} />)
    ...
    const addNewHandler = () => {
        ...
        props.history.push('/admin/gallery/addnew');
    }

    const onRefreshHandler = async () => {
        try {
            loadingtableHandler(true);
            let alldatas = await dispatch(getGallery());
            let totaldata = await alldatas.payload.galleries.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
            var mydata = [];
            var resultsdata = [];
            var offset = (currentPage - 1) * pageSize;
            alldataHandler(totaldata);
            resultsdata = totaldata.map((item, index) => <GalleryTable key={index} item={item} index={index} />)
            mydata = resultsdata.slice(offset, offset + pageSize);
            mydataHandler(mydata);
            dataCountHandler(resultsdata.length);
            loadingtableHandler(false);
    ...
    const editDataHandler = () => {
        props.history.push({
            pathname: '/admin/gallery/editdata',
            state: {
